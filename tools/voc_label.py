...
sets = [('2020', 'train'), ('2020', 'val'), ('2020', 'test')]  # 替换为自己的数据集
...
def convert_annotation(year, image_id, classes):
    ...
    out_file = open('trash_labels/%s.txt' % (image_id), 'w')
...
def gen_voc_lable(classes):
    for year, image_set in sets:
        ...
        if not os.path.exists('trash_labels/'):
            os.makedirs('trash_labels/')
        ...
        for image_id in image_ids:
            image_id = image_id[:-1]
            print(image_id)
            # list_file.write('./data/images/%s2007/%s.jpg\n' % (image_set, image_id))
            list_file.write('./data/images/%s2020/%s.png\n' % (image_set, image_id))
